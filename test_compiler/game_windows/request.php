...
<script type="text/javascript">
    ...
    function compileAjax() {
        ...
        getRequestCompile(
            'exec_windows.php', // URL for the PHP file
...
<script type="text/javascript">
    function periodicalCheckCompilingStatus(){
        ...
        interval = setInterval(function() {
            var reqMonitor = new XMLHttpRequest();
            reqMonitor.onreadystatechange = function() {

                console.log("onread" + reqMonitor.readyState);

                if (reqMonitor.readyState == 4) {
                    if (reqMonitor.responseText.length != counterCharsPrevious) {

                        counterChars = reqMonitor.responseText.length;
                        document.getElementById("reporto").innerHTML = counterChars + " lines";

                        counterCharsPrevious = reqMonitor.responseText.length;
                    } else {
                        document.getElementById("reporto").innerHTML = "Compiling lasted: " + (new Date().getTime() - start_time)/1000 + " seconds";

                        if (reqMonitor.responseText.indexOf("Exiting batchmode successfully now!")>0){
                            document.getElementById("reporto2").innerHTML = "and the result is Success.";
                            clearInterval(interval);
                            myzipajax();
                        } else {
                            clearInterval(interval);
                            document.getElementById("reporto2").innerHTML = "and the result is Error [15] : HTML " + reqMonitor.status;
                        }
                    }
                }
            };

            reqMonitor.open("POST", "stdout.log", true); //Use POST to avoid caching
            reqMonitor.send();
        }, 1000);
    }
</script>
